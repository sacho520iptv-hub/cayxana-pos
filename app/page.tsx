...
export default function Home() {
  ...
  // İlk açılışda məlumatları yüklə
  useEffect(() => {
    setMounted(true);
    const savedStollar = localStorage.getItem('abideler-stollar');
    const savedHistory = localStorage.getItem('abideler-history');
  ...
  useEffect(() => {
    if (mounted && stollar.length > 0) {
      localStorage.setItem('abideler-stollar', JSON.stringify(stollar));
      localStorage.setItem('abideler-history', JSON.stringify(history));
    }
  }, [stollar, history, mounted]);
  ...
  const calculateTotal = (stol: Stol) => {
    return stol.orders.reduce((sum, item) => {
      // Sadə Çay: 1-ci 2 AZN, digərləri 1 AZN
  ...
  const closeOrder = () => {
    if (!selectedStol || !selectedStol.ofisiant) {
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-zinc-100 font-sans">
      <header className="sticky top-0 z-40 bg-zinc-900/80 backdrop-blur-md border-b border-zinc-800 px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
             <button onClick={clearData} className="p-2 text-zinc-600 hover:text-red-500 transition-colors">
                <RefreshCw size={20} />
             </button>
             <button 
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {showHistory ? (
            ...
            <div className="flex justify-between items-end mb-8">
              <h2 className="text-3xl font-black italic">GÜNLÜK KASSA</h2>
              <p className="text-4xl font-black text-green-500">{dailyTotal.toFixed(2)} AZN</p>
            </div>
                ...
                <tbody className="divide-y divide-zinc-800">
                  {history.map((sale, i) => (
                    <tr key={i} className="hover:bg-zinc-800/30">
                      ...
                      <td className="p-5 font-bold">Stol #{sale.stolId}</td>
                      <td className="p-5 text-zinc-300 font-medium">{sale.ofisiant}</td>
                      <td className="p-5">
                        <div className="flex flex-wrap gap-1">
                          {sale.items.map((item, idx) => (
                            <span key={idx} className="bg-zinc-800 text-[10px] px-2 py-1 rounded border border-zinc-700 text-zinc-400">
                              {item.name} (x{item.quantity})
                            </span>
                          ))}
                        </div>
                      </td>
                      <td className="p-5 text-right font-black text-orange-400 text-lg">{sale.total.toFixed(2)} ₼</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {stollar.map((stol) => (
              ...
              >
                <div className="w-full flex justify-between items-start">
                    <span className={`text-3xl font-black ${stol.status === 'occupied' ? 'text-orange-500' : 'text-zinc-800'}`}>
                    {stol.id}
                    </span>
                    {stol.status === 'occupied' && stol.ofisiant && (
                        <span className="text-[9px] bg-orange-600 text-white px-2 py-1 rounded font-bold uppercase">
                            {stol.ofisiant}
                        </span>
                    )}
                </div>
                {stol.status === 'occupied' ? (
                  ...
                  <span className="text-xs text-zinc-700 font-bold uppercase tracking-widest">Boş</span>
                )}
              </button>
            ))}
          </section>
      ...
      {selectedStol && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4">
          <div className="absolute inset-0 bg-black/90 backdrop-blur-sm" onClick={() => setSelectedStol(null)} />
          
          <div className="relative bg-zinc-900 w-full max-w-6xl h-full sm:h-[90vh] sm:rounded-[40px] shadow-2xl overflow-hidden flex flex-col border border-zinc-800 animate-in zoom-in duration-200">
            <div className="flex items-center justify-between p-6 border-b border-zinc-800">
              <div className="flex items-center gap-6">
                ...
                <div className={`flex items-center gap-2 p-1 pl-3 rounded-xl border transition-all ${!selectedStol.ofisiant ? 'bg-red-500/10 border-red-500/50' : 'bg-zinc-800 border-zinc-700'}`}>
                    ...
                    className="bg-transparent text-sm font-bold py-2 pr-4 outline-none min-w-[150px] text-white"
                    >
              ...
              <button onClick={() => setSelectedStol(null)} className="w-12 h-12 flex items-center justify-center bg-zinc-800 rounded-2xl hover:bg-zinc-700 transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="flex flex-1 flex-col lg:flex-row overflow-hidden">
              <div className="flex-1 p-6 overflow-y-auto custom-scrollbar">
                <div className="relative mb-6">
                  ...
                  <input 
                    ...
                    className="w-full bg-zinc-800/50 rounded-2xl py-4 pl-12 pr-4 outline-none border border-zinc-800 focus:border-orange-500 text-white"
                    value={searchTerm}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {filteredMenu.map(product => (
                    <button
                      ...
                      className="bg-zinc-800/40 hover:bg-orange-500 border border-zinc-800 p-5 rounded-[24px] text-left transition-all active:scale-95 group"
                    >
                      <p className="font-bold text-base mb-1 group-hover:text-white">{product.name}</p>
              ...
              <div className="w-full lg:w-[400px] bg-black/10 p-6 flex flex-col border-l border-zinc-800">
                <h3 className="text-xl font-black mb-6 flex items-center gap-2">
                  <LayoutGrid size={20} className="text-orange-500" /> SİFARİŞ SİYAHISI
                </h3>

                <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                  {selectedStol.orders.map(item => (
                    <div key={item.uniqueId} className="bg-zinc-800/60 p-4 rounded-2xl border border-zinc-700/50">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-1 bg-black/20 rounded-xl p-1 border border-zinc-700">
                          <button onClick={() => changeQuantity(item.uniqueId, -1)} className="w-8 h-8 flex items-center justify-center font-bold hover:bg-zinc-700 rounded-lg">-</button>
                ...
                <div className="mt-6 pt-6 border-t border-zinc-800">
                  {!selectedStol.ofisiant && selectedStol.orders.length > 0 && (
                      <p className="text-red-500 text-[11px] font-bold uppercase mb-3 animate-pulse text-center">Ofisiant seçilməyib!</p>
                  )}
                  <div className="flex justify-between items-end mb-6">
                    <span className="text-zinc-500 font-bold uppercase text-xs tracking-widest">Cəmi Məbləğ</span>
                  ...
                  <button 
                    ...
                    className={`w-full py-5 rounded-[22px] font-black text-lg transition-all ${!selectedStol.ofisiant ? 'bg-zinc-800 text-zinc-600 cursor-not-allowed' : 'bg-green-600 hover:bg-green-500 text-white shadow-xl shadow-green-950/20'}`}
                  >
                    HESABI BAĞLA
                  </button>
                </div>
